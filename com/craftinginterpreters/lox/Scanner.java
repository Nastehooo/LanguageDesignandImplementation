...
class Scanner {
    // A map to store all reserved keysowrds in the Lox language and their
    ...
    private void identifier() {
        while (isAlphaNumeric(peek()))
            advance(); // Consume rest of the identifier

        String text = source.substring(start, current);
        TokenType type = keywords.get(text); // Check if it's a keyword
        if (type == null)
            type = IDENTIFIER; // If not, it's a normal identifier
        addToken(type);
    }
